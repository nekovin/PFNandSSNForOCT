...
def get_fpss_model(model_name, train_config, loss_name, num_epochs, learning_rate, optim, checkpoint):
    ...
    if model_name == 'SSMSimple':
        if train_config['load_model']:
            ...
            model = get_fpss_model_simple(checkpoint_path=checkpoint_path)
            model.to(device)
            ...
            model = get_fpss_model_simple(checkpoint_path=None)
            model.to(device)
            optimizer = optim.Adam(model.parameters(), lr=learning_rate)
            best_loss = float('inf')
            set_epoch = 0
    ...
    if model_name == 'SSMAttention':
        if train_config['load_model']:
            checkpoint_path = train_config['checkpoint'].format(loss_fn=loss_name)
            model = get_fpss_model_attention(checkpoint_path=checkpoint_path)
            model.to(device)
            optimizer = optim.Adam(model.parameters(), lr=learning_rate)
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            best_loss = checkpoint['best_loss']
            ...
            model = get_fpss_model_attention(checkpoint_path=None)
            model.to(device)
            optimizer = optim.Adam(model.parameters(), lr=learning_rate)
            best_loss = float('inf')
            set_epoch = 0
